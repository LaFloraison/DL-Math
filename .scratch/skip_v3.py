...
print("=== Demo 1: gradient norm reaching each layer (depth=20) ===")
...
def train(X, y, depth, width, residual, steps=1500, lr=0.05, seed=0):
    Ws, bs = init_net(depth, width, seed)
    for t in range(steps):
        hs = forward(X, Ws, bs, residual)
        pred = np.tanh(hs[-1].mean(axis=1))       # scalar readout: mean over units
...
print("\n=== Demo 2: fit random labels, depth=15, 1500 steps ===")
for residual in (False, True):
